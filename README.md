...
本目录保存全英／全欧中国经济学会（Chinese Economic Association UK/Europe，CEA）的公众号写作与论文深度解读工作流，包括可安装的 Codex Skill、历史训练语料、参考排版 Skill、品牌图片和完整样例。
...
| 论文深度解读 | 阅读笔记、文献综述素材、理论与方法拆解、证据评价 | 一份干净的论文解读正文及一份编辑核查记录 |

如果请求只写“论文解读”，Skill 会先询问需要“公众号文章”还是“论文阅读笔记”，不会默认混合两种结构。
...
- 内部区分作者观点、论文证据、作者解释和编辑判断，正文用自然主语表达归属；
- 围绕核心问题挑选理论、方法、结果、图表、贡献与局限，不逐节复述；
- 将关键判断定位到 Section、Figure、Table、Equation、Appendix 或 PDF 页码；
- 正文通常只用 4—6 个二级标题，结构连贯、篇幅适中；
- 工作流状态、判断映射和未决问题单独写入编辑核查记录，不进入读者正文。
...
- [论文解读工作流](cea-wechat-writer/references/research-paper-analysis-workflow.md)
- [论文解读启动 Prompt](cea-wechat-writer/assets/prompts/START_RESEARCH_PAPER_ANALYSIS.md)
- [自然写作与干净正文规则](cea-wechat-writer/references/natural-writing-and-clean-copy.md)
...
现在只收集输入并执行阶段 1，不得开始完整论文解读或生成最终输出文件。
...
- 默认生成 `<论文PDF文件名去除扩展名>_论文解读.md` 和 `<论文PDF文件名去除扩展名>_编辑核查记录.md`；
- 阶段 1 不创建文件；
- 已有同名文件时必须询问，不得静默覆盖。
...
阶段 1 必须完整读取 `SKILL.md` 及其指定的 references、templates、scripts 和 assets，并检查 PDF、Markdown、figures 与两个输出路径。回复只包含：

#### A. 对任务的理解

说明两份最终产出、三类论文材料的用途、核心约束和拟保存路径。

#### B. 预定 Markdown 结构

按论文实际研究类型提出少量宽口径结构，默认只用 4—6 个二级标题，不强行给非因果研究套用 DID/RCT 结构。

#### C. 需要确认的事项

默认建议：

- 重点导向的完整解读，篇幅适中且不逐节复述；
- 中文为主，专业术语首次出现时保留英文；
- 平衡覆盖问题、理论、方法、证据、贡献与局限；
- 默认不扩展外部文献；
- 只深入分析真正支撑主线的核心图表；
- 正文自然区分判断归属，详细证据映射写入编辑核查记录。

语言仍须明确选择中文、英文、中英文或其他。选择中英文时，在正文前确认全部缩写的英文全称、中文译名和首次及后续展示规则；选择“其他”时，确认具体语言、地区变体或文字体系，以及单语还是与中文并列。

#### D. 文件检查结果

报告 Skill、PDF、Markdown、figures 和两个输出路径的读取状态、格式异常、内容对应关系与阻断项。
...
只分析影响核心结论的图表，但不在 Markdown 中嵌入图片。
完成缩写确认后开始阶段 2。
```
...
- 重要判断提供原文定位；
- 无法核实且影响理解的内容用自然句说明，详细状态写入编辑核查记录；
- 作者观点、论文证据和编辑判断通过具体主语自然区分；
- 正文不出现工作流标签、审核状态、Prompt/Skill 说明或过细的模板化标题。

中文正文通常约 3,000—5,000 个中文字符，复杂论文一般不超过约 6,000 个中文字符；需要更长时先征得用户同意。其他语言保持相同信息密度，不机械换算字符数。

交付说明应列出正文和编辑核查记录的完整路径、使用的源文件、无法解析或确认的内容、Skill 检查状态，以及 Prompt 与 Skill 冲突的处理方式。
...
- 读者正文不显示工作流标签、审核状态、Prompt/Skill 说明或写作过程解释；
- 作者观点、论文证据和编辑判断通过具体主语自然区分，不使用“分析者评价”等机械标签；
- 只在主题真正变化时使用标题，合并一段式小节，减少连续列表、过度粗体和重复总结；
- 审核状态、来源映射、冲突和未决问题写入配套审校文件。

完整规则见 [`natural-writing-and-clean-copy.md`](cea-wechat-writer/references/natural-writing-and-clean-copy.md)。
...
论文解读模式默认输出两份 Markdown：供读者直接阅读的正文，以及只供作者和编辑核查的记录。
