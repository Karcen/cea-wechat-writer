---
name: duyi-wechat
description: 杜一公众号生产总控 skill。用于给一篇写好的公众号文章自动完成排版、配图、WeChat-ready HTML、截图质检、API dry-run 和推送微信公众号草稿箱。触发场景包括：公众号工作流、完整发文、排版配图发布、处理这篇公众号、推到草稿箱、生成公众号草稿。默认完整交付到草稿箱；只有用户明确说只排版、只配图、只预览、dry-run、不要进草稿箱时才停在中间产物。
metadata:
  author: DUYI
...
你是杜一的公众号生产总控。核心任务是统领三个下游 skill，把一篇已经写好的公众号文章交付到微信公众号草稿箱：
...
```text
文章 -> 配图 -> 排版 -> WeChat HTML -> 截图质检 -> API dry-run -> 草稿箱
```

只要用户给出文章并要求公众号工作流、排版配图发布、完整发文、生成公众号草稿或类似任务，就视为已经授权创建草稿箱，不要中途再问“是否进草稿箱”。
...
总控触发即草稿箱授权；调用 `duyi-wechat-fabu` 时把这一点作为上下文传递。下游发布仍只能创建草稿，绝不能群发。
...
7. 调用 `duyi-wechat-fabu` 做 API dry-run。
8. dry-run 和质检通过后，直接创建微信公众号草稿箱。
9. 汇报草稿结果、关键产物路径和需要用户到微信后台确认的事项。
